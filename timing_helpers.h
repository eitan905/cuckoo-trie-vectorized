...
#define ENABLE_TIMING
    ...
    #define UPDATE_FIND_BY_COLOR_STATS(start_cycles, total, count, min, max, hist, is_secondary, cell_idx) do { \
        uint64_t cycles = rdtsc_stop() - start_cycles; \
        total += cycles; \
        count++; \
        if (cycles < min) min = cycles; \
        ...
        if (is_secondary) { \
            ...
            find_by_color_secondary_cell_counts[cell_idx]++; \
        } else { \
            find_by_color_primary_bucket++; \
            find_by_color_primary_cell_counts[cell_idx]++; \
        } \
    } while(0)
#else
    ...
    #define UPDATE_FIND_BY_COLOR_STATS(start_cycles, total, count, min, max, hist, is_secondary, cell_idx) do { } while(0)
#endif
